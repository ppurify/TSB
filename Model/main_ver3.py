...
import make_arc_ver3 as make_arc
import network_LP
...
def main(_grid, _YT_locations, _Job_locations, number_of_YT, number_of_Job, casefolder_path, time, _A1_star, _A2_star, _A3_star, alpha1, alpha2, alpha3, rep):
  ...
  arcs_YT_to_Pick, arcs_Pick_to_Drop, arcs_Drop_to_Pick, arcs_Drop_to_Sink, arcs_YT_to_Sink = make_arc.create_arcs(
      ...
      grid=_grid, _A1_star=_A1_star, _A2_star =_A2_star, _A3_star = _A3_star, now_count=now_count, time_consumed_per_grid=time_consumed_per_grid, processing_time=processing_time)
  ...
  # 다음 스케줄링 A1이랑 겹치는 부분 : _A1_star
  # 다음 스케줄링 A2이랑 겹치는 부분 : _A2_star 
  _A1_star = np.zeros((len(grid), len(grid[0])))  
  _A2_star = np.zeros((len(grid), len(grid[0])))
  _A3_star = np.zeros((len(grid), len(grid[0])))
  
  for arc in activated_arcs:
    if arc.i[0] == 'YT':      
      # arc.path에 있는 모든 좌표에 1씩 더해줌
      for i in range(len(arc.path)):
        _A1_star[arc.path[i][0]][arc.path[i][1]] += 1
        
    elif arc.i[0] == 'Pick':
      for i in range(len(arc.path)):
        _A2_star[arc.path[i][0]][arc.path[i][1]] += 1
  
    elif arc.i[0] == 'Drop' and arc.j[0] == 'Pick':
      for i in range(len(arc.path)):
        _A3_star[arc.path[i][0]][arc.path[i][1]] += 1
  ...
  with open(log_file_path, 'w') as f:
    ...
    _A1_star_str = np.array2string(_A1_star, separator=', ').replace('.', '')
    _A2_star_str = np.array2string(_A2_star, separator=', ').replace('.', '')
    _A3_star_str = np.array2string(_A3_star, separator=', ').replace('.', '')
    f.write("A1_star count : \n" + _A1_star_str + "\n")
    f.write("A2_star count : \n" + _A2_star_str + "\n")
    f.write("A3_star count : \n" + _A3_star_str + "\n")
      
  return _A1_star, _A2_star, _A3_star
  ...
  for _ in range(len(Prev_number_list)):
    ...
    for rep in range(reps):
      ...
      for i in range(len(alphas[0])):
        ...
        # Prev
        A1_star = np.zeros((len(grid), len(grid[0])))
        A2_star = np.zeros((len(grid), len(grid[0])))
        A3_star = np.zeros((len(grid), len(grid[0])))

        A1_star, A2_star, A3_star = main(grid, prev_YT_locations, prev_Job_locations, Prev_number_of_YT, Prev_number_of_Job, case_folder_path, 'prev', A1_star, A2_star, A3_star, alpha1, alpha2, alpha3, rep)
        main(grid, now_YT_locations, now_Job_locations, Now_number_of_YT, Now_number_of_Job, case_folder_path, 'now', A1_star, A2_star, A3_star, alpha1, alpha2, alpha3, rep)