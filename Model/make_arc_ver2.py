...
def penalty(normalized_prev_count, route, number_of_final_route, alpha1, alpha3, time_consumed_per_grid):
    ...
    for i in range(len(route)):
        ...
        # print('penalty')
        # print(alpha1 * sum_of_counter_of_prev_count + alpha3 * (sum_of_move * time_consumed_per_grid))
...
def create_arcs(YT_locations, Job_locations, number_of_final_route, alpha1, alpha2, alpha3, grid, prev_count, now_count, time_consumed_per_grid, processing_time):
    ...
    for j in range(len(Job_locations)):
        ...
        if len(route_Pick_to_Drop) > number_of_final_route:
            final_route_Pick_to_Drop = penalty(A1_grid, route_Pick_to_Drop, number_of_final_route, alpha1=alpha1, alpha3=alpha3, time_consumed_per_grid=time_consumed_per_grid)
        else:
        ...
        for j in range(len(Job_locations)):
            if i != j:
                ...
                if len(route_Drop_to_Pick) > number_of_final_route :
                    final_route_Drop_to_Pick = penalty(A2_grid, route_Drop_to_Pick, number_of_final_route, alpha1=alpha1, alpha3=alpha3, time_consumed_per_grid=time_consumed_per_grid)
                else:
    ...
    arcs_YT_to_Pick, arcs_Pick_to_Drop, arcs_Drop_to_Pick, arcs_Drop_to_Sink, arcs_YT_to_Sink, now_count = sort_and_cost(YT_locations, Job_locations, arcs_YT_to_Pick, arcs_Pick_to_Drop, arcs_Drop_to_Pick, arcs_Drop_to_Sink, arcs_YT_to_Sink, prev_count, now_count, alpha1, alpha2, alpha3, time_consumed_per_grid, processing_time)
