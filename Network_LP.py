...
Total_arc_num = Agv_num + sum(Agv_to_pick) + sum(Pick_to_drop) + Drop_to_sink
...
for i in range(Total_arc_num):
    Arc_status[i] = solver.NumVar(0, 1, 'Arc_status[%i]' % i)


# Constraint 1 
for i in range(Agv_num):
    solver.Add(sum(Arc_status[j] for j in Arc_bundles[i]) == 1)
